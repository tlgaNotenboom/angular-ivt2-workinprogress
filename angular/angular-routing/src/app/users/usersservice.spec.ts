...
describe('Usersservice', () => {
  beforeEach(() => {
    ...
    usersservice = new UserService(<any> httpClientSpy);
  });
  ...
  it('return from httpClient', () => {
    httpClientSpy.get.and.returnValue(asyncData(users));

    usersservice.AllUsersFromApi().subscribe(
      returnedUsers => expect(returnedUsers).not.toBeNull()
    );

  });

  it('calls http client', () => {
    httpClientSpy.get.and.returnValue(asyncData(users));
    ...
    expect(httpClientSpy.get.calls.count()).toBe(1, 'Webservice should only be called once by the service');
  });
